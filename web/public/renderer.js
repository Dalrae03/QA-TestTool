...
function saveFolderAssignment(tcId, isNew = false) {
  ...
  // 폴더 배정 PATCH의 프로미스를 반환해 호출부(handleSubmit)가 목록 재조회 전에 await 할 수 있게 한다.
  // (미await 시, 원격 백엔드 지연에서 재조회가 PATCH 반영보다 먼저 끝나 새 TC가 선택 폴더에서 누락됨)
  return request(`/api/testcases/${tcId}/folder`, { method: "PATCH", body: JSON.stringify({ folderId }) }).catch(() => {});
}
...
async function handleSubmit(event) {
  ...
  try {
    if (id) {
      ...
      await saveFolderAssignment(updated.id);
      ...
      await saveFolderAssignment(created.id, true);  // isNew: 선택된 폴더 자동 배정
